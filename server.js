...
setInterval(() => {
    if(rouletteState.status === 'BETTING') {
        ...
        if(rouletteState.timeLeft <= 0) {
            ...
            // TIMING:
            // Spin (4s Client) + Hold (1.5s) + Table Reveal (1.5s) + Gather (2s) + Coin (1.5s) = ~10.5s total
            // We set server timeout to match the end of visuals before resetting
            ...
            setTimeout(() => {
                ...
                }, 7000); // 7s buffer for post-spin animations (Gather + Coin)
            }, 5000); // 4s Spin + Buffer
        }
    }
}, 1000);
